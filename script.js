...
function renderPickerList(){
  const searchQRaw = (document.getElementById("crypto-search")?.value || "");
  const searchQ = searchQRaw.trim();
  const listEl = document.getElementById("crypto-list");
  if (!listEl) return;

  const fav = CRYPTO_ASSETS.filter(a => cryptoFav.has(a.id));
  const nonFav = CRYPTO_ASSETS.filter(a => !cryptoFav.has(a.id));

  // Product behavior: first load should never look "empty".
  // If user has no favorites and no search, show a curated top list instead of a blank panel.
  const TOP = ["BTCUSDT","ETHUSDT","SOLUSDT","XRPUSDT","BNBUSDT","ADAUSDT","DOGEUSDT","AVAXUSDT","LINKUSDT","MATICUSDT","DOTUSDT","LTCUSDT"];
  const topList = TOP.map(id => findAsset(CRYPTO_ASSETS, id)).filter(Boolean);

  if (!searchQ && fav.length === 0) {
    listEl.innerHTML =
      renderGroup({ title:"Top", items:topList, activeId:currentCrypto, favSet:cryptoFav, searchQ }) +
      renderGroup({ title:"All", items:nonFav, activeId:currentCrypto, favSet:cryptoFav, searchQ });
    return;
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Hard fail-safe: GitHub Pages is a clean origin (no localStorage). Ensure we always have a valid default symbol.
  if (!findAsset(CRYPTO_ASSETS, currentCrypto)) {
    currentCrypto = "BTCUSDT";
    setLast(LS_KEYS.lastCrypto, currentCrypto);
  }
  if (typeof LightweightCharts === "undefined") {
    console.error("LightweightCharts not loaded.");
    const el = document.getElementById("chart");
    if (el) el.innerHTML = '<div style="padding:18px;color:#e5e7eb;font-size:14px;">Chart library failed to load (lightweight-charts). Check network / script tag.</div>';
    return;
  }
